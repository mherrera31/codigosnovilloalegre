...
def render_user_management():
    ...
    roles = db_service.get_roles(); branches = db_service.get_branches()
    ...
    branch_options_with_none = {"Ninguna / N/A": None}; branch_options_with_none.update(branch_options)
            ...
            if submitted_create:
                ...
                if not error:
                    if create_user_profile(input_email, input_username, input_password, role_id, branch_id, phone_number):
                        st.session_state['create_user_form_key_counter'] += 1; st.rerun()
            ...
            for index, user_row in df_users.iterrows():
                ...
                with st.expander(expander_title, key=f"exp_user_{user_id}"):
                    ...
                    with st.form(edit_form_key):
                        ...
                        with cols_buttons[1]: submitted_delete = st.form_submit_button("Eliminar Usuario (Auth y Perfil)", key=f"delete_{user_id}")
